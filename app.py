...
# --- API: Login ---
@app.route('https://visionflow-backend.onrender.com/login', methods=['POST', 'OPTIONS'])
def login():
    if request.method == 'OPTIONS':
        return ('', 204)
    data = request.get_json()
...
# --- API: Chat (AI) ---
@app.route('https://visionflow-backend.onrender.com/chat', methods=['POST', 'OPTIONS'])
def chat():
    if request.method == 'OPTIONS':
        return ('', 204)
    data = request.get_json()
    ...
    if tool == "logo":
        ...
        try:
            ...
            # Try to extract brand name and business type from the message
            import re
            brand_name = None
            business_type = None
            # Look for 'for a ... brand called ...' or 'for a ... business called ...'
    ...
    try:
        ...
        # Chat history saving removed as requested
...
# --- API: Save Favorite ---
@app.route('https://visionflow-backend.onrender.com/save-favorite', methods=['POST', 'OPTIONS'])
def save_favorite():
    if request.method == 'OPTIONS':
        return ('', 204)
    data = request.get_json()
...
# --- API: View Favorites ---
@app.route('https://visionflow-backend.onrender.com/favorites', methods=['POST', 'OPTIONS'])
def view_favorites():
    if request.method == 'OPTIONS':
        return ('', 204)
    data = request.get_json()
...
# --- API: Delete Favorite ---
@app.route('https://visionflow-backend.onrender.com/delete-favorite', methods=['POST', 'OPTIONS'])
def delete_favorite():
    if request.method == 'OPTIONS':
        return ('', 204)
    data = request.get_json()
...
# --- API: Clear Favorites ---
@app.route('https://visionflow-backend.onrender.com/clear-favorites', methods=['POST', 'OPTIONS'])
def clear_favorites():
    if request.method == 'OPTIONS':
        return ('', 204)
    data = request.get_json()
...
@app.route('https://visionflow-backend.onrender.com/send-message', methods=['POST'])
def send_message():
    data = request.json
    name = data.get('name')
    email = data.get('email')
