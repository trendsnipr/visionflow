...
# Utility: Clean web output (keeps unique lines and adds HTML wrapper)
...
def add_waitlist():
    ...
    # TODO: Implement saving email to DB or list
...
def register():
    ...
    # Email deliverability check via mails.so API
    try:
...
def chat():
    ...
    tool = data.get('tool')
    email = data.get('email')
    plan = data.get('plan', 'starter')  # default
    ...
    if tool == "logo":
        ...
        # TODO: Implement usage count for pro plan
...
# ---------------------
# API: Proxy Image (download remote image safely)
# ---------------------
...
def send_message():
    ...
    # Email verification via mails.so
    try:
        mails_api_key = "b9c66737-97ba-455a-b310-260b3735e96d"
        mails_url = f"https://api.mails.so/v1/validate?email={email}"
        mails_headers = {"x-mails-api-key": mails_api_key}
